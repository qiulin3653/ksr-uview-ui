		...
		<view :id="id">
			<scroll-view scroll-x class="u-scroll-view" :scroll-left="scrollLeft" scroll-with-animation>
				<view class="u-scroll-box" :class="{'u-tabs-scorll-flex': !isScroll}">
					<view class="u-tab-item u-line-1" :class="{'u-tab-item-active':index===currentIndex}" :id="'u-tab-item-' + index" v-for="(item, index) in list" :key="index" @tap="clickTab($event,index)" :style="[tabItemStyle(index)]">
		...
		methods: {
			...
			getTabOne() {
				return new Promise((resolve, reject)=>{
					// 创建节点查询
					let query = uni.createSelectorQuery().in(this);
					query.select(`#u-tab-item-${this.currentIndex}`).fields({
						size: true,
						dataset: true,
						rect: true,
						scrollOffset: true
